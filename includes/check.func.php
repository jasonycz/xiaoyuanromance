...
function _check_username($_string,$_min_num,$_max_num) {
	...
	//限制敏感用户名    通过|来分割出来写入数据库中的敏感用户名 explode();得到的是一个数组
	$_mg = explode('|',$_system['sensitiveString']);
	//告诉用户，有哪些不能够注册
...
/**
 * _check_nickname表示检测并过滤用户昵称
 * @access public
 * @param string $_string 受污染的昵称
 * @param int $_min_num  最小位数
 * @param int $_max_num 最大位数
...
function _check_nickname($_string,$_min_num,$_max_num){
	if (!empty($_string)) {
			...
			//这里采用的绝对匹配
			if (in_array($_string,$_mg)) {
				_alert_back($_mg_string.'以上敏感昵称不得注册');
			}		
			//将用户名转义输入
			return _mysql_string($_string);
		} else {
...
/**
 * _check_type_one 必填的类型 进行转义
 * @access public
 * @param string $_string
 * @return string
 */

function _check_type_one($_string) {
	return _mysql_string($_string);
}
/**
 * _check_type_two  不是必填的类型  进行转义
 * @access public
 * @param string $_string
 * @return string
 */
function _check_type_two($_string){
	if(!empty($_string)){
		return _mysql_string($_string);
	}else{
		return null;
...
function _check_content($_string) {
	if (mb_strlen($_string,'utf-8') < 1 || mb_strlen($_string,'utf-8') > 1000) {
		_alert_back('内容请不要小于1位或者大于1000位！');
	}
	return $_string;
}
...
function _check_photo_descript($_string,$_max) {
	if ( mb_strlen($_string,'utf-8') > $_max ) {
		_alert_back('照片描述不能大于'.$_max.'字数！');
	}
	return _mysql_string($_string);
}

function _check_photography_name($_string) {
//	ini_set('pcre.backtrack_limit', 999999999);
	//限制敏感字符-<> \'\"我为什么设置了pcre.backtrack_limit=-1还是不行?????preg_match还是有长度限制
// 	$_char_pattern = '/[~!,@#$%&^*_?-<> \'\"]/';
// 	if (preg_match($_char_pattern,$_string)) {
// 		_alert_back('相册的名字不合理不能包含~,!,@,#,$,%,&,^,*,_,-,?,<,>,逗号，双引号');
// 	}
